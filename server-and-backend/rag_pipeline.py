...
try:
    ...
    5. [중요] "PDF에 따르면", "문서에 의하면", "상세 설명 인용"과 같은 표현은 **절대 사용하지 마십시오.** 마치 당신이 직접 알고 있는 지식인 것처럼 자연스럽게 서술하십시오.
    ...
    [출력 형식 가이드]
    당신의 답변은 **반드시** 아래의 마크다운(Markdown) 형식을 그대로 따라야 합니다. 
    다른 사족(예: "네, 알겠습니다")을 붙이지 말고, 아래 서식의 (괄호) 내용을 채워 보고서를 완성하십시오.

    # (매력적인 여행 보고서 제목)

    ## 1. 추천 요약
    (고객에게 전하는 정중한 인사말과 전체적인 추천 이유를 요약)
    ...
    - 상세 설명:
      (PDF 내용을 바탕으로 한 상세 설명. "PDF에 따르면", "문서에 의하면" 같은 말 없이 자연스럽게 서술할 것)
    ...
    (상품이 더 있다면 위와 동일한 형식으로 추가)

    ## 3. 종합 의견
    (전문가로서의 마무리 의견 및 여행 팁)
    """
