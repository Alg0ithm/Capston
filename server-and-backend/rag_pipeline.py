...
try:
    ...
    4. 반드시 프롬프트 하단에 있는 [출력 형식 가이드]를 엄격하게 준수하여 작성하십시오.
    5. [중요] "PDF에 따르면", "문서에 의하면" 등의 인용구는 절대 사용하지 마십시오.
    6. [가독성] 문단과 문단 사이, 리스트 항목 사이에는 **반드시 '빈 줄(Empty Line)'을 하나씩 추가**하여 빽빽하지 않게 작성하십시오. (엔터를 두 번 치는 효과)
    7. [평점] 만약 제공된 정보에 '평점'이 없다면, 보고서의 완성도를 위해 3.8 ~ 4.55점 사이의 점수를 적절히 부여하여 작성하십시오.
    8. **[분할] 서버 처리를 위해 '각 상품 설명' 사이에는 반드시 `<<<SPLIT>>>` 이라는 구분자를 별도의 줄에 작성하십시오.**
    ...
    [출력 형식 가이드]
    1. 당신의 답변은 **반드시** 아래의 마크다운(Markdown) 형식을 그대로 따라야 합니다.
    2. **가독성을 위해 모든 항목과 단락 사이에는 '두 번의 줄바꿈'을 사용하여 확실한 공백(빈 줄)을 넣으십시오.**
    3. **각 섹션(요약, 개별 상품, 결론)이 끝날 때마다 `<<<SPLIT>>>`을 입력하여 내용을 분리하십시오.**
    4. 다른 사족(예: "네, 알겠습니다")을 붙이지 말고, 바로 내용부터 작성을 시작하십시오.

    
    🏆 (상품명 1) / 평점: ⭐ (점수)
    
    - 추천 포인트: (이 상품의 핵심 장점을 굵게 강조)
    
    
    - 상세 설명:
      (PDF 내용을 바탕으로 한 상세 설명. "PDF에 따르면" 같은 말 없이 자연스럽게 서술할 것. 문장이 길어지면 적절히 줄을 바꾸어 작성.)
    ...
    (상품이 더 있다면 위와 동일한 형식으로 <<<SPLIT>>>을 넣어 추가)
